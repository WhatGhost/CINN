...
TEST(GenerateCode_Cpu, Norm) {
  ...
  int axis       = -1;
  float epsilon  = 1e-12;
  ir::Tensor res = Norm(in1, axis, epsilon, "test_norm_out");

  poly::StageMap stages = poly::CreateStages({res});
  std::vector<ir::LoweredFunc> funcs =
      lang::LowerVec("TestGenerateCodeCpu_Norm", stages, {res}, {}, {}, nullptr, target, true);

  VLOG(6) << "Expr before CPU codegen:";
  VLOG(6) << funcs[0]->body;

  ir::Module::Builder builder("Norm_Module", target);
  for (auto& f : funcs) {
...
TEST(GenerateCode_Gpu, Norm) {
  ...
  std::vector<ir::LoweredFunc> funcs =
      lang::LowerVec("TestGenerateCodeCuda_Norm", stages, {res}, {}, {}, nullptr, target, true);
